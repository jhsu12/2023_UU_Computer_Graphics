...
struct Context {
    ...
    bool is_textcoordinate;
    gltf::TextureList textures;
    bool is_texturemapping;
};
...
void draw_scene(Context &ctx)
{
    ...
    //texture coordinate
    glUniform1f(glGetUniformLocation(ctx.program, "is_textcoordinate"), ctx.is_textcoordinate);

    //texture mapping 
    glUniform1f(glGetUniformLocation(ctx.program, "is_texturemapping"), ctx.is_texturemapping);
    ...
    for (unsigned i = 0; i < ctx.asset.nodes.size(); ++i) {
        ...
        if (mesh.primitives[0].hasMaterial) 
        {
            ...
            if (pbr.hasBaseColorTexture) 
            {
                ...
                glUniform1i(glGetUniformLocation(ctx.program, "u_sampler"), 1);
            } 
...
int main(int argc, char *argv[])
{
    ...
    bool is_textcoordinate = false;
    bool is_texturemapping = false;
    ...
    while (!glfwWindowShouldClose(ctx.window)) {
        ...
        // ImGui
        {
            ...
            if (ImGui::BeginTabBar("")) 
            {
                ...
                if(ImGui::BeginTabItem("Misc"))
                {
                    ...
                    ImGui::Checkbox("Show Texture Coordinates", &is_textcoordinate);
                    ImGui::Checkbox("Show Texture Mapping", &is_texturemapping);
                    ImGui::EndTabItem();
                }
        ...
        ctx.is_textcoordinate = is_textcoordinate;
        load_cubemap(ctx, cubemapNames[cubemapIndex]);
        
        ctx.is_texturemapping = is_texturemapping;
